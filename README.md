<img src="https://i.namu.wiki/i/LGDe7Mgmt6h-be49CNzuuRqH5i2DfRXfHkxk1fvXgawBiBHSHxy-Kq6vXQLgxzbrLfTvVHCp0Uh8EZndVYPWcA.gif" width="150" alt="사용자 지정 GIF">

### 🌟 소개

* **이름:** Gyeong-creator
* **전공:** 컴퓨터•인공지능공학부 24th
* **관심 있는 개발 분야:**
    * **게임 개발 (Game Development):** 사용자에게 즐거움을 선사하는 인터랙티브 콘텐츠 개발
    * **정보 보안 (Information Security):** 안전한 시스템 및 데이터 보호 기술 연구

---

### 🛠️ 기술 스택

| Category | Skills |
| :--- | :--- |
| **Languages** | ![C](https://img.shields.io/badge/c-%2300599C.svg?style=flat&logo=c&logoColor=white) ![C++](https://img.shields.io/badge/c++-%2300599C.svg?style=flat&logo=c%2B%2B&logoColor=white) ![Python](https://img.shields.io/badge/python-3670A0?style=flat&logo=python&logoColor=ffdd54) ![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=flat&logo=javascript&logoColor=%23F7DF1E) |
| **Frontend** | ![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=flat&logo=html5&logoColor=white) ![CSS3](https://img.shields.io/badge/css3-%231572B6.svg?style=flat&logo=css3&logoColor=white) |
| **Backend** | ![Flask](https://img.shields.io/badge/flask-%23000.svg?style=flat&logo=flask&logoColor=white) |
| **Database** | ![MySQL](https://img.shields.io/badge/mysql-%2300f.svg?style=flat&logo=mysql&logoColor=white) |
| **Tools** | ![Git](https://img.shields.io/badge/git-%23F05033.svg?style=flat&logo=git&logoColor=white) ![GitHub](https://img.shields.io/badge/github-%23121011.svg?style=flat&logo=github&logoColor=white) |

---

### 📫 연락처

* **Email:** kmkr2005@naver.com

---

### 🚀 현재 진행 중인 프로젝트 소개

### 알바하기싫다 - 웹 기반의 쉽고 직관적인 가계부 서비스

| 구분 | 내용 |
| :---: | :--- |
| **핵심 설명** | 간단한 수입/지출 기록 및 소비 패턴을 시각화하는 가계부 웹 페이지 제작 |
| **프로젝트 목적** | 사용자가 자신의 재정을 **쉽게 관리**하고 **소비 패턴을 직관적으로 파악**할 수 있도록, 웹 기반의 간편한 가계부 서비스 제공 |
| **GitHub Repository** | [https://github.com/Gyeong-creator/-5-](https://github.com/Gyeong-creator/-5-) |

---

### 👨‍💻 프로젝트 역할 - "알바하기싫다"

이 프로젝트에서 저는 **풀스택 개발자** 역할을 수행하며 백엔드 로직 구현 및 데이터베이스 관리에 중점을 두고 있습니다.

* **프론트엔드 개발:** 메인 페이지(입금/출금 내역 조회 화면)의 기본적인 UI 구조 및 레이아웃 구현.
* **백엔드 개발:** Flask를 이용한 라우팅 관리, 가계부 내역 등록/삭제 기능 서버 로직 구현.
* **데이터베이스 관리:** 가계부 내역 등을 저장하기 위한 MySQL 테이블 설계 및 CRUD 관리.
* **문서 작성:** 프로젝트 팀원 간의 협업 및 행동 규범(Code of Conduct) 작성.

---

### 📊 GitHub 활동 내역

#### 🏆 GitHub Stats & Profile Trophy

[![Anurag's GitHub stats](https://github-readme-stats.vercel.app/api?username=Gyeong-creator&show_icons=true&theme=buefy&count_private=true)](https://github.com/anuraghazra/github-readme-stats)
[![GitHub Profile Trophy](https://github-profile-trophy.vercel.app/?username=Gyeong-creator&theme=onedark)](https://github.com/ryo-ma/github-profile-trophy)

[![Gyeong-creator's github activity graph](https://github-readme-activity-graph.vercel.app/graph?username=Gyeong-creator&theme=react-dark)](https://github.com/ashutosh00710/github-readme-activity-graph)

#### 💡 현재 활동 내역 및 기여

* **참여 중인 오픈소스 프로젝트:** 알바하기싫다 (가계부 웹 서비스 제작)
* **최근 기여 내역 (Pull Request):**
    * **기여 내용:** 스타일시트(CSS) 통합 및 프로젝트 내 사용 언어 통일 작업
    * **링크:** [Pull Request #69](https://github.com/Gyeong-creator/-5-/pull/69)

---

### 🗺️ My Development Roadmap

| 구분 | 학습 목표 (Goal) | 주요 기술 (Tech) | 상태 (Status) |
| :---: | :---: | :---: | :---: |
| **게임 개발** | 3D 게임 엔진 이해 및 활용 | **Unity/Unreal Engine**, C# | 📚 To Study |
| **정보 보안** | 웹 취약점 분석 및 방어 | OWASP Top 10, **Kali Linux**, WAF | 📚 To Study |
| **백엔드** | 서비스 확장성을 고려한 서버 구조 학습 | **Node.js** 또는 **Django**, Docker | 📚 To Study |
